import RangeUtil from '@models/RangeUtil'
...
export default class Editor {
    ...
    buttonHandler(event) {
        ...
        if (this.selection?.anchorNode) {
            this.selectedTag = this.selectedTag === event.currentTarget.dataset.tag ? null : event.currentTarget.dataset.tag
    ...
    markSelection() {
        ...
        if (this.selectedTag) {
            ...
            return RangeUtil.highlight(this.selectedTag, this.selection, '', attr);
        }
    ...
    highlightSelectedButton() {
        let allFocusNodeParentNode = RangeUtil.getAllParentNode(this.selection.focusNode)
        let allAnchorNodeParentNode = RangeUtil.getAllParentNode(this.selection.anchorNode)
        let allGeneralParentNode = intersect(allFocusNodeParentNode, allAnchorNodeParentNode)

        const selectedMarkTags = allGeneralParentNode.filter((node) => {
